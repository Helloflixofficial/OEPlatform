...
export default function TeacherVideoRoomPage({ params }: PageProps) {
  ...
  useEffect(() => {
    if (!isLoaded) return;
    if (!userId) {
      router.push(`/sign-in?redirect_url=/teacher/meet/${roomName}`);
      return;
    }
    fetchToken();
  ...
  const endMeeting = useCallback(async () => {
    if (!window.confirm("End this meeting for everyone? Participants will be disconnected.")) return;
    try {
      const sessionsRes = await fetch(`/api/livekit/sessions?roomName=${encodeURIComponent(roomName)}`);
      if (sessionsRes.ok) {
        const sessions = await sessionsRes.json();
        const session = sessions?.[0];
        if (session?.id) {
          await fetch(`/api/livekit/sessions/${session.id}`, { method: "DELETE" });
        }
      }
    } finally {
      router.push("/teacher/meet");
    }
  }, [roomName, router]);
  ...
  if (loading) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-zinc-950 gap-4">
        ...
        <p className="text-zinc-400 text-sm">{!isLoaded ? "Checking your account…" : "Connecting as host…"}</p>
      </div>
    );
  }
