...
const CreateSessionModal = ({
  ...
  onClose: () => void;
  onCreated: (session: MeetingSession, startNow: boolean) => void;
}) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 border border-slate-200">
        ...
        <label className="flex items-center gap-2 mt-5 text-xs text-slate-600 cursor-pointer select-none">
          <input
            type="checkbox"
            checked={startNow}
          ...
          />
          Open the meeting room immediately after creating it
        </label>
        ...
        <div className="flex gap-3 mt-6">
          ...
          >
            {loading ? (
              ...
              <><Video className="h-4 w-4 mr-2" /> {startNow ? "Create & Start" : "Create Session"}</>
            )}
          </Button>
        </div>
      </div>
...
const SessionCard = ({
  ...
  // Use the current HTTPS origin when the dashboard is opened securely. This
  // keeps shared LAN invites on HTTPS, which browsers require for media APIs.
  ...
  useEffect(() => {
    ...
    const isLoopback = (hostname: string) => /^(localhost|127\.0\.0\.1|::1)$/i.test(hostname);
    const host = isLoopback(current.hostname) && !isLoopback(configured.hostname)
      ? configured.hostname
      : current.hostname;
    const protocol = current.protocol === "https:" ? "https:" : configured.protocol;
    const port = current.port || configured.port;
    setAppUrl(`${protocol}//${host}${port ? `:${port}` : ""}`);
  }, [configuredAppUrl]);

  const joinUrl = `${appUrl.replace(/\/$/, "")}/meet/${session.roomName}`;
  const inviteNeedsHttps = appUrl.startsWith("http://") && !/localhost|127\.0\.0\.1/.test(appUrl);
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        {session.isActive && inviteNeedsHttps && (
          <p className="-mt-2 mb-4 text-[11px] leading-relaxed text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
            Camera, microphone, and screen sharing need an HTTPS invite on another device. Run <span className="font-mono">npm run dev:https</span>, open the HTTPS address, then copy the new link.
          </p>
        )}
...
export default function TeacherMeetPage() {
  ...
  const [view, setView] = useState<"all" | "active" | "ended">("all");
  ...
  const handleCreated = (session: MeetingSession, startNow: boolean) => {
    setSessions((prev) => [session, ...prev]);
    if (startNow) handleJoin(session.roomName);
  };

  const handleJoinInput = () => {
    const value = joinValue.trim();
    if (!value) {
      toast.error("Enter a room code or meeting link");
      return;
    }
    try {
      const parsed = new URL(value, window.location.origin);
      const match = parsed.pathname.match(/\/meet\/([^/]+)/);
      handleJoin(match?.[1] || value.replace(/^room[-\s]*/i, "room-"));
    } catch {
      handleJoin(value.replace(/^room[-\s]*/i, "room-"));
    }
  };
  ...
  const visibleSessions = sessions.filter((session) => {
    const matchesView = view === "all" || (view === "active" ? session.isActive : !session.isActive);
    const query = search.trim().toLowerCase();
    const matchesSearch = !query || `${session.title} ${session.description || ""} ${session.roomName}`.toLowerCase().includes(query);
    return matchesView && matchesSearch;
  });
  const visibleActiveSessions = visibleSessions.filter((session) => session.isActive);
  const visibleEndedSessions = visibleSessions.filter((session) => !session.isActive);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        {/* Quick actions */}
        <div className="grid grid-cols-1 lg:grid-cols-[1.05fr_1fr] gap-4 mb-8">
          <button
            onClick={() => setShowModal(true)}
            className="group text-left rounded-2xl p-5 bg-gradient-to-br from-sky-600 to-indigo-600 text-white shadow-lg shadow-sky-200/60 hover:shadow-xl hover:-translate-y-0.5 transition-all"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-sky-100">
                  <Sparkles className="h-3.5 w-3.5" /> Quick start
                </span>
                <h2 className="text-lg font-bold mt-2">Start a live class now</h2>
                <p className="text-sm text-sky-100 mt-1 max-w-sm">Create a room, preview your devices, and invite students in seconds.</p>
              </div>
              <span className="rounded-xl bg-white/15 p-2.5 group-hover:bg-white/25 transition-colors">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-sm font-semibold text-slate-800">Join an existing room</h2>
            </div>
            <div className="flex gap-2">
              <Input
                value={joinValue}
                ...
                placeholder="Paste a meeting link or room code"
                className="h-10 border-slate-200 bg-slate-50 text-sm"
              />
              <Button onClick={handleJoinInput} className="h-10 bg-slate-900 hover:bg-slate-800 text-white px-4">
                Join
              </Button>
            </div>
            <p className="text-[11px] text-slate-400 mt-2">Example: room-a1b2c3 or /meet/room-a1b2c3</p>
          </div>
        </div>
        ...
        {!loading && sessions.length > 0 && (
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-5">
            <div className="flex items-center gap-1 rounded-xl bg-white border border-slate-200 p-1 w-fit">
              {([
                ["all", "All sessions"],
                ["active", "Live now"],
                ["ended", "Ended"],
                ...
                <button
                  key={key}
                  onClick={() => setView(key)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${view === key ? "bg-slate-900 text-white" : "text-slate-500 hover:text-slate-800 hover:bg-slate-50"}`}
                >
                  {label}
                </button>
              ))}
            ...
            <div className="relative w-full md:w-64">
              ...
              <Input
                ...
                className="h-9 pl-9 border-slate-200 bg-white text-xs"
              />
            </div>
          </div>
        )}
        {/* Active Sessions */}
        {!loading && visibleActiveSessions.length > 0 && (
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <h2 className="text-sm font-semibold text-slate-600 uppercase tracking-wide">
                Active Sessions ({visibleActiveSessions.length})
              </h2>
            </div>
        ...
        {/* Ended Sessions */}
        {!loading && visibleEndedSessions.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-4">
              <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wide">
                Ended Sessions ({visibleEndedSessions.length})
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {visibleEndedSessions.map((session) => (
                <SessionCard
                  key={session.id}
                  session={session}
                  onDelete={handleDelete}
